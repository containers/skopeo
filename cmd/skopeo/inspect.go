...
func manifestDigestFromManifest(manifestBlob []byte, img types.Image, userAlgorithm digest.Algorithm) (digest.Digest, error) {
	if userAlgorithm != "" {
		...
		return userAlgorithm.FromBytes(manifestBlob), nil
	}
	...
	if configInfo.Digest != "" {
		...
		if alg.Available() {
			return alg.FromBytes(manifestBlob), nil
		}
	}
...
func (a *algorithmValue) Set(value string) error {
	...
	if err := supporteddigests.TmpSetDigestForNewObjects(algorithm); err != nil {
		return err
	}
